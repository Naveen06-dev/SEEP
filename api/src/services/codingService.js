...
export async function runVisibleTestCases({ questionId, language, sourceCode }) {
  ...
  if (!question.codingCount && question.exam?.codingCount === 0) {
    // no-op guard handled at exam level
  }

  const allowed = question.allowedLanguages;
  if (Array.isArray(allowed) && !allowed.includes(language)) {
...
export async function submitCodingAnswer({ attemptId, questionId, language, sourceCode }) {
  ...
  for (const tc of question.testCases) {
    ...
    testResults.push({ weight: tc.weight, passed, isHidden: tc.isHidden });
  }

  const scoring = calculateWeightedScore(testResults, question.marks);
